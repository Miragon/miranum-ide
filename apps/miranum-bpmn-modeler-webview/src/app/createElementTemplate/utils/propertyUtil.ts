...
//
// export function getPropertyValue(element: any, property: any) {
...
//
//         if (taskDefinition) {
//             if (type === CAMUNDA_DELEGATE_EXPRESSION_TYPE) {
...
//         const ioMapping = findExtension(businessObject, "camunda:inputOutput");
//
//         if (!ioMapping) {
//             return defaultValue;
//         }
...
//
//         if (zeebeProperties) {
//             const zeebeProperty = findCamundaProperty(zeebeProperties, binding);
//
//             if (zeebeProperty) {
...
//     // zeebe:taskDefinition
//     if (TASK_DEFINITION_TYPES.includes(type)) {
//         const oldTaskDefinition = findExtension(
//             extensionElements,
//             "zeebe:TaskDefinition",
//         );
//
...
//             .get("values")
//             .filter((v: any) => v !== oldTaskDefinition);
//
//         commands.push({
//             cmd: "element.updateModdleProperties",
//             context: {
...
//
//     if (IO_BINDING_TYPES.includes(type)) {
//         let ioMapping = findExtension(extensionElements, "camunda:inputOutput");
//
//         if (!ioMapping) {
...
//             const values = ioMapping
//                 .get("inputParameters")
//                 .filter((v: any) => v !== oldZeebeInputParameter);
//
//             // do not persist empty parameters when configured as <optional>
//             if (shouldUpdate(value, property)) {
//                 const newZeebeInputParameter = createInputParameter(
//                     binding,
//                     value,
//                     bpmnFactory,
//                 );
...
//                 .filter((v: any) => v !== oldZeebeOutputParameter);
//
//             // do not persist empty parameters when configured as <optional>
//             if (shouldUpdate(value, property)) {
//                 const newZeebeOutputParameter = createOutputParameter(
//                     binding,
//                     value,
//                     bpmnFactory,
//                 );
...
//
//         const values = taskHeaders.get("values").filter((v: any) => v !== oldTaskHeader);
//
...
//         }
//
//         const oldZeebeProperty = findCamundaProperty(zeebeProperties, binding);
//
//         const properties = zeebeProperties
//             .get("properties")
//             .filter((p: any) => p !== oldZeebeProperty);
//
//         if (shouldUpdate(value, property)) {
...
// }
//
// export function unsetProperty(commandStack: any, element: any, property: any) {
//     let businessObject = getBusinessObject(element);
//
//     const { binding } = property;
//
...
//     // camunda:inputOutput
//     if (IO_BINDING_TYPES.includes(type)) {
//         const ioMapping = findExtension(extensionElements, "camunda:inputOutput");
//
//         if (!ioMapping) return;
//
//         // zeebe:Input
...
//             const values = ioMapping
//                 .get("inputParameters")
//                 .filter((value: any) => value !== oldZeebeInputParameter);
//
...
//             const values = ioMapping
//                 .get("outputParameters")
//                 .filter((value: any) => value !== oldZeebeOutputParameter);
//
...
//     // zeebe:taskHeaders
//     if (type === ZEEBE_TASK_HEADER_TYPE) {
//         const taskHeaders = findExtension(extensionElements, "zeebe:TaskHeaders");
//
//         if (!taskHeaders) return;
//
//         const oldTaskHeader = findTaskHeader(taskHeaders, binding);
//
//         const values = taskHeaders
//             .get("values")
//             .filter((value: any) => value !== oldTaskHeader);
//
//         if (values.length === 0) {
//             commands.push({
//                 cmd: "element.updateModdleProperties",
...
// ) {
//     const { constraints = {}, label } = property;
//
//     const { maxLength, minLength, notEmpty } = constraints;
//
...
//
// // helpers
// function unknownBindingError(element: any, property: any) {
//     const businessObject = getBusinessObject(element);
//
...
//
//     return new Error(
//         `unknown binding <${type}> for element <${id}>, this should never happen`,
//     );
// }
//
// /*
...
// }
//
// function matchesPattern(string: string, pattern: RegExp | string) {
