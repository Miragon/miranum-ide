...
import { default as defaultTranslate } from "diagram-js/lib/i18n/translate/translate";

import { isString, isUndefined, without } from "min-dash";
...
export function unsetProperty(commandStack, element, property) {
    ...
    if (IO_BINDING_TYPES.includes(type)) {
        ...
        if (type === CAMUNDA_INPUT_TYPE) {
            ...
            const values = ioMapping
                .get("inputParameters")
                .filter((value) => value !== oldZeebeInputParameter);
        ...
        if (type === CAMUNDA_OUTPUT_TYPE) {
            ...
            const values = ioMapping
                .get("outputParameters")
                .filter((value) => value !== oldZeebeOutputParameter);
    ...
    if (type === ZEEBE_TASK_HEADER_TYPE) {
        ...
        const values = taskHeaders
            .get("values")
            .filter((value) => value !== oldTaskHeader);
