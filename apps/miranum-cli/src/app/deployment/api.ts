 ...
 * The file in itself consists out of a name, an extension, some content, a size, and a path
 *
 * @example
 * ``` bash
    ...
    return new Command()
        ...
        .action((options) => {
            mapProcessConfigToDigiwfLib().then((digiwfLib) => {
                ...
                deployment
                    .deployArtifact(options.file, options.type, options.target)
                    .then(() => console.log("Deployment was successfully"))
                    .catch((err) =>
                        console.log(colors.red.bold("FAILED") + ` with -> ${err}`),
                    );
            });
        });
}
...
/**
 * This command deploys all artifacts to the target environment.
...
export function deployAllFiles(): Command {
    return new Command()
        ...
        .requiredOption(
            ...
            "specify the directory of the source files",
        )
        .requiredOption("-t, --target <target>", "specify the target environment")
        .action((options) => {
            mapProcessConfigToDigiwfLib(options.directory).then((digiwfLib) => {
                const deployment = new Deployment(digiwfLib);
                deployment
                    .deployAllArtifacts(options.directory, options.target)
                    .then(() =>
                        console.log(
                            `Project ${digiwfLib.projectConfig?.name} was successfully deployed to environment ${options.target}`,
                        ),
                    )
                    .catch((err) =>
                        console.log(colors.red.bold("FAILED") + ` with -> ${err}`),
