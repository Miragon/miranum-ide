...
const customDeployment = new Deployment(
    createMiranumCore(
        "0.0.1",
        "my-process-automation-project",
        [
...
describe("deployArtifact", () => {
    for (const file of filesToDeploy) {
        it(`${file.type} should work with standard settings`, async () => {
            ...
            await expect(
                deployment.deployArtifact(file.path, file.type, sampleTarget),
    ...
    it(`should work with custom Artifact`, async () => {
        ...
        const logSpy = jest.spyOn(console, "log");
        await expect(
            customDeployment.deployArtifact(
                customForm.path,
                customForm.type,
                sampleTarget,
            ),
    ...
    it(`should not work, due to unsupported type`, async () => {
        const eTemplate = {
            nameExt: "test.json",
            path: "resources/my-process-automation-project/element-templates/test.json",
            type: "element-template",
        };
        return deployment
            .deployArtifact(eTemplate.path, eTemplate.type, sampleTarget)
            .catch((e) =>
                expect(e).toBe(`${eTemplate.type} is not supported for deployment`),
            );
    });

    it("should raise an error", async () => {
        return deployment
            .deployArtifact(pathToProject, "BPMN", sampleTarget)
...
describe("deployAllArtifacts", () => {
    it("my-process-automation-project should work", async () => {
        const logSpy = jest.spyOn(console, "log");
        await expect(
            customDeployment.deployAllArtifacts(pathToProject, sampleTarget),
        ).resolves.not.toThrow();
    ...
    it("should raise an error, due to wrong path", async () => {
        return deployment
            .deployAllArtifacts("/path/does-not/exist", sampleTarget)
            .catch((e) => expect(e).not.toBeNull());
    });
});
