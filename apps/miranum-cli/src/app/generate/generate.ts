...
export class ProjectGenerator {
    ...
    public async generateFile(name: string, type: string, path: string): Promise<void> {
        const artifact = await this.digiwfLib.generateArtifact(
            name,
            type,
            this.digiwfLib.projectConfig?.name ?? "",
            path,
        );
        await this.generate(artifact, path);
    }
    ...
    private async generate(artifact: Artifact, path: string): Promise<void> {
        try {
            ...
            fileName =
                fileName.charAt(0) === "/"
                    ? fileName.slice(1, fileName.length)
                    : fileName;
            console.log(colors.green.bold("SAVED ") + fileName);
        } catch (err) {
            console.log(
                colors.red.bold("FAILED ") +
                    ` creating file ${artifact.file.name} with -> ${err}`,
            );
